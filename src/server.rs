...
use tracing::trace_simple_print;
...
impl<F: FileSystem + Sync> Server<F> {
    ...
    pub fn handle_message<R: Reader + ZeroCopyReader, W: Writer + ZeroCopyWriter, M: Mapper>(
        ...
        trace_simple_print!("fuse server: handle_message: in_header={:?}", in_header);
    ...
    fn init<R: Reader, W: Writer>(&self, in_header: InHeader, mut r: R, w: W) -> Result<usize> {
        trace_simple_print!("fuse server: init: in_header={:?}", in_header);
        let InitIn {
            major,
            minor,
            max_readahead,
    ...
    fn readdirplus<R: Reader, W: Writer>(
        ...
        trace_simple_print!("fuse server: readdirplus: in_header={:?}", in_header);
        let ReadIn {
            fh, offset, size, ..
        } = zerocopy_from_reader(&mut r).map_err(Error::DecodeMessage)?;
